...
function randomClientSeed() {
  ...
  return "client-" + Array.from(bytes).map(b => b.toString(16).padStart(2, "0")).join("");
}
// END TEST //
...
export default function DiceGame({
 ...
 {/* ORIGINAL async function startRound() {
  ...
  // 👇 fresh clientSeed for THIS bet
  const seedsForRound = {
    ...seeds,
    clientSeed: randomClientSeed(),
    nonce: Number(seeds.nonce) || 1,
  };
  // optional: reflect it in the UI so you can see it changed
  setSeeds(seedsForRound);

  let rid: string | null = null, hash = "";
  if (onPlaceBet) {
    try {
      const resp = await onPlaceBet(bet, { mode, chance }, seedsForRound);
      rid = resp?.roundId ?? null;
      hash = resp?.serverSeedHash ?? "";
    } catch (e) { console.warn("onPlaceBet failed; demo mode.", e); }
  } else {
    const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(seedsForRound.serverSeed));
    hash = bufferToHex(digest);
  }
  setRoundId(rid);
  setServerSeedHash(hash);
  ...
  setRolling(true);
  const roll = await nextRoll100(seedsForRound); // uses the fresh clientSeed
  const didWin = mode === "under" ? roll < threshold : roll > threshold;
  await new Promise((r) => setTimeout(r, 350));
  setResult(roll); setWin(didWin); setRolling(false);
  // Show marker and reset its hide timer
...
markerTimerRef.current = setTimeout(() => {
  ...
  setLastRoll(roll); setLastWin(didWin);
  // newest first; keep 10 items (change to 14 if you prefer)
  setRecent((prev) => [{ v: roll, win: didWin }, ...prev].slice(0, 10));
